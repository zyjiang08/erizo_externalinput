...
namespace erizo {
    ...
    int init_input_frame(AVFrame **frame)
    {
        if (!(*frame = av_frame_alloc())) {
            ELOG_DEBUG("Could not allocate input frame");
            ...
            AVCodecContext *output_codec_context)
    {
        ...
        resample_context = swr_alloc_set_opts(NULL,
                ...
                0, NULL);
        if (!resample_context) {
            ELOG_DEBUG( "Could not allocate resample context\n";
                                ...
                                ELOG_DEBUG( "av_assert0 done\n";
                                    ...
                                    ELOG_DEBUG( "Could not open resample context\n";
                                        ...
                                        ELOG_DEBUG( "swr_init done\n";
                                            ...
                                            ELOG_DEBUG("Could not reallocate FIFO\n";
                                                return error;
                                                }
                                                ...
                                                int frame_size)
                                        {
                                                ...
                                                ELOG_DEBUG( "Could not allocate output frame\n";
                                                        ...
                                                        if ((error = av_frame_get_buffer(*frame, 0)) < 0) {
                                                            ELOG_DEBUG( "Could allocate output frame samples (error '%s')\n"<<get_error_text(error);
                                                                    ...
                                                                    static int encode_package_audio_frame(AVFrame *frame, unsigned char* outbuf)
                                                                    {
                                                                    ...
                                                                    if ((error = avcodec_encode_audio2(output_codec_context, &output_packet,
                                                                        ...
                                                                        ELOG_DEBUG("Could not encode frame (error '%s')\n"<<get_error_text(error));
                                             ...
                                             * output file.
                                             */
                                            static int load_encode_and_write(unsigned char* outbuf)
                                            {
                                                ...
                                                {
                                                    ELOG_DEBUG(" init_output_frame failed!!");
                                                    return 0;
                                                }
                                                /**
                                                ...
                                                if (av_audio_fifo_read(fifo, (void **)output_frame->data, frame_size) < frame_size) {
                                                    ELOG_DEBUG(("Could not read data from FIFO\n");
                                                            ...
                                                            if (pktlen <= 0)
                                                            {
                                                            ELOG_DEBUG(" Failed to encode_audio_frame!!");
                                                            }
                                                            av_frame_free(&output_frame);
                                                    ...
                                                    int frame_size)
                                            {
                                                ...
                                                if (!(*converted_input_samples = (uint8_t**)calloc(output_codec_context->channels,
                                                    ...
                                                    ELOG_DEBUG( "Could not allocate converted input sample pointers\n";
                                                            return AVERROR(ENOMEM);
                                                            }
                                                            ...
                                                            ELOG_DEBUG("Could not allocate converted input samples "<<get_error_text(error);
                                                                ...
                                                                int AudioDecoder::initDecoder (const AudioCodecInfo& info){
                                                                    codec_ = avcodec_find_decoder(static_cast<AVCodecID>(info.codec));
                                                                    if (!codec_) {
                                                                        ELOG_DEBUG("Audio decoder not found");
                                                                        return false;
                                                                    }

                                                                    output_codec_context = avcodec_alloc_context3(codec_);
                                                                    ...
                                                                    output_codec_context->bit_rate = info.bitRate;
                                                                    output_codec_context->sample_rate = info.sampleRate;
                                                                    output_codec_context->channels = 1;

                                                                    if (avcodec_open2(output_codec_context, codec_, NULL) < 0) {
                                                                ...
                                                                int AudioDecoder::initDecoder (AVCodecContext* context, AVCodec* dec_codec){
                                                                    ...
                                                                    if (!input_codec_context) {
                                                                        ELOG_DEBUG(("AudioDecoder Error allocating audio decoder context");
                                                                                ...
                                                                                //input_codec_context->sample_fmt = AV_SAMPLE_FMT_S16;
                                                                                //input_codec_context->bit_rate = 64000;
                                                                                //input_codec_context->sample_rate = 48000;
                                                                                ...
                                                                                int AudioDecoder::decodeAudio(AVPacket& input_packet, unsigned char* outbuf)
                                                                                {
                                                                                    if (1)
                                                                                    {
                                                                                        // just forward, for test
                                                                                        return packageAudio(input_packet.data, input_packet.size, outbuf);
                                                                                    }
                                                                                    ...
                                                                                    if (error < 0)
                                                                                    {
                                                                                        ELOG_DEBUG((__func__)<<get_error_text(error));
                                                                                    ...
                                                                                    {
                                                                                        ELOG_DEBUG(" convert_samples failed!!");
                                                                                    ...
                                                                                    if (add_samples_to_fifo(fifo, converted_input_samples,
                                                                                                input_frame->nb_samples))
                                                                                    {
                                                                                        ELOG_DEBUG(" add_samples to fifo failed !!");
                                                                                ...
                                                                                int AudioDecoder::decodeAudio(unsigned char* inBuff, int inBuffLen,
                                                                                    ...
                                                                                    while (avpkt.size > 0) {
                                                                                        ...
                                                                                        len = avcodec_decode_audio4(output_codec_context, &frame, &got_frame,
                                                                                                &avpkt);
                                                                                        if (len >= 0 && got_frame) {
                                                                                            int plane_size;
